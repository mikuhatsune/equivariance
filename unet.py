...
class UNet_up_block(nn.Module):
    ...
    def forward(self, prev_feature_map, x):
        ...
        if len(x) != len(prev_feature_map):
            n = len(x) - len(prev_feature_map)
            n = len(prev_feature_map) // n
            prev_feature_map = torch.cat((prev_feature_map, prev_feature_map[::n]))
...
class UNet(nn.Module):
    def __init__(self, downsample=6, in_channels=3, out_channels=3, last_act=None):
        super().__init__()
