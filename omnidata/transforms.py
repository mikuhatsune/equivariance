...
def get_transform(task: str, image_size=Optional[int]):
    ...
    elif task in ['mask_valid', 'segment_unsup2d', 'segment_unsup25d']:
...
#         return transform_16bit_int
    ...
    # elif task in FEED_FORWARD_TASKS:
...
# transform_dense_labels = lambda img: torch.from_numpy(np.array(img, dtype=np.int32))
transform_dense_labels = lambda img: torch.from_numpy(np.array(img)).long().unsqueeze(0)

# Transforms to a 3-channel tensor and then changes [0,1] -> [-1, 1]
transform_8bit = transforms.ToTensor()
