...
def make_mask(tensor, task):
    return build_mask(tensor.unsqueeze(0), val=TASK_PARAMETERS[task]['mask_val'])[0]
...
def build_mask(target, val=0.0, tol=1e-3, mask_extra_radius=DEFAULT_MASK_EXTRA_RADIUS):
    # print(target.shape, target.dtype, val)
    if target.dtype is torch.int64 or target.dtype is torch.int32:
        mask = target.ne(val)
        return mask

    if target.shape[1] == 1:
        # mask = ((target >= val - tol) & (target <= val + tol))
        mask = (target - val).abs() <= tol
    ...
    # mask = (t >= val - tol) & (t <= val + tol)
    mask = (target - val).abs() <= tol
    mask = mask.prod(dim=1, keepdim=True)
    mask = (0 == F.conv2d(mask.float(),
                        torch.ones(1, 1, mask_extra_radius, mask_extra_radius, device=mask.device),
                        padding=(mask_extra_radius // 2)) )
    return mask
